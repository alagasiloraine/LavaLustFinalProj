...
//   }
    ...
    public function dashboard() {
        ...
        $currentWeekCounts = [
            'jobSeekers' => is_array($currentWeek['jobSeekers']) ? count($currentWeek['jobSeekers']) : 0,
            'employers' => is_array($currentWeek['employers']) ? count($currentWeek['employers']) : 0,
            'users' => is_array($currentWeek['users']) ? count($currentWeek['users']) : 0,
            'jobs' => is_array($currentWeek['jobs']) ? count($currentWeek['jobs']) : 0,
            'appliedApplications' => is_array($currentWeek['appliedApplications']) ? count($currentWeek['appliedApplications']) : 0,
            'hiredApplications' => is_array($currentWeek['hiredApplications']) ? count($currentWeek['hiredApplications']) : 0,
            'inactiveJobs' => is_array($currentWeek['inactiveJobs']) ? count($currentWeek['inactiveJobs']) : 0,
        ];

        $previousWeekCounts = [
            'jobSeekers' => is_array($previousWeek['jobSeekers']) ? count($previousWeek['jobSeekers']) : 0,
            'employers' => is_array($previousWeek['employers']) ? count($previousWeek['employers']) : 0,
            'users' => is_array($previousWeek['users']) ? count($previousWeek['users']) : 0,
            'jobs' => is_array($previousWeek['jobs']) ? count($previousWeek['jobs']) : 0,
            'appliedApplications' => is_array($previousWeek['appliedApplications']) ? count($previousWeek['appliedApplications']) : 0,
            'hiredApplications' => is_array($previousWeek['hiredApplications']) ? count($previousWeek['hiredApplications']) : 0,
            'inactiveJobs' => is_array($previousWeek['inactiveJobs']) ? count($previousWeek['inactiveJobs']) : 0,
        ];
...
//   }
    ...
    public function analytics() {
        ...
        // Function to check if the result is an array or object and count the items
        function safeCount($result) {
            return is_array($result) || is_object($result) ? count($result) : 0;
        }

        $currentWeekCounts = [
            'jobs' => safeCount($currentWeek['jobs']),
            'appliedApplications' => safeCount($currentWeek['appliedApplications']),
            'hiredApplications' => safeCount($currentWeek['hiredApplications']),
            'inactiveJobs' => safeCount($currentWeek['inactiveJobs']),
        ];

        $previousWeekCounts = [
            'jobs' => safeCount($previousWeek['jobs']),
            'appliedApplications' => safeCount($previousWeek['appliedApplications']),
            'hiredApplications' => safeCount($previousWeek['hiredApplications']),
            'inactiveJobs' => safeCount($previousWeek['inactiveJobs']),
        ];
